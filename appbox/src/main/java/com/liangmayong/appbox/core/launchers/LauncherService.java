...
import com.liangmayong.appbox.core.app.AppClassLoader;
import com.liangmayong.appbox.core.app.AppConstant;
import com.liangmayong.appbox.core.app.AppContext;
...
public class LauncherService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        String dexPath = intent.getStringExtra(AppConstant.INTENT_APP_PATH);
        if (dexPath == null || "".equals(dexPath)) {
            return super.onStartCommand(intent, flags, startId);
        }
        String launchName = intent.getStringExtra(AppConstant.INTENT_APP_ACTIVITY);
        ...
        if (!serviceKeys.contains(dexPath + ":" + launchName)) {
            try {
                baseService = (Service) AppClassLoader.getClassloader(dexPath).loadClass(launchName).newInstance();
            } catch (Exception e) {
            }
     ...
     */
    private void copyTo(Object oldObject, Object targetObject, String appPath) {
        ...
        if (fieldValues != null && !fieldValues.isEmpty()) {
            for (Map.Entry<String, Object[]> entry : fieldValues.entrySet()) {
                ...
                if (ContextWrapper.class.getName().equals(value[1]) && "mBase".equals(key)) {
                    ...
                    setField(targetObject, key, AppContext.get(context, appPath));
                } else {
